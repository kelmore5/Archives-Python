...
def toString(tree, level=0):
  ...
  return '' if (tree == EMPTY) else toString(tree.top, level+1) + up +'  '*level+ str(tree.cargo) + '\n' + down + toString(tree.bottom, level+1)

def makeTree ( list , n=1 ) :
  return EMPTY if (n > len(list)) else Tree( list[n-1] , makeTree( list , 2 * n ) , makeTree ( list, 2 * n + 1 )) 
...
def sumOfNum(tree):
	sumNum = tree.cargo
	if tree.top != None:
		sumNum += sumOfNum(tree.top)
	if tree.bottom != None:
		sumNum += sumOfNum(tree.bottom)
	return sumNum
...
def sumOfNum2(tree):
	if tree == EMPTY:
		...
		return tree.cargo + sumOfNum2(tree.top) + sumOfNum2(tree.bottom)
	...
	while ptr != EMPTY:
		if key == ptr.cargo:
			if ptr.top == EMPTY and ptr.bottom == EMPTY:
				...
				if prevTempPtr == None and prevPtr != None:
					if top:
						prevPtr.top = tempPtr
					else:
...
myBST = makeBST([10,8,4,5,6,2,1,3,7,9,15,16,13,19,14,17,12,11,18])
#print(myBST)
